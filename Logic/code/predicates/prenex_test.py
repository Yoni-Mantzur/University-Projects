...
def test_is_quantifier_free(debug=True):
    for formula,free in [
            ('x=y', True),
            ('R(x,y)', True),
            ('Ax[x=y]', False),
...
def test_is_in_prenex_normal_form(debug=True):
...
def test_make_quantified_variables_unique(debug=True):
    for formula in [
        '(~x=z&z=x)'
        'Ax[Q(x,y)]',
        'Q(x,c)',
...
def test_pull_out_quantifications_across_negation(debug=True):
    for formula,expected in [
        ...
        if debug:
            print('... got', result)
        a = proof.conclusion == equivalence_of(formula, result)
...
def test_pull_out_quantifications_from_left_across_binary_operator(debug=True):
    for formula,expected in [
...
def test_pull_out_quantifications_from_right_across_binary_operator(debug=True):
    for formula,expected in [
        ('(Q(x,c)|R(d,y))', '(Q(x,c)|R(d,y))'),
        ('(S()&Ax[T(x)])', 'Ax[(S()&T(x))]'),
        ('(S()&Ex[T(x)])', 'Ex[(S()&T(x))]'),
...
def test_pull_out_quantifications_across_binary_operator(debug=True):
    for formula,expected in [
        ('(Q(x,c)|R(d,y))', '(Q(x,c)|R(d,y))'),
...
def test_to_prenex_normal_form_from_unique_quantified_variables(debug=True):
    for formula,pnf in [
        ('Q(x,c)', 'Q(x,c)'),
        ('Ax[Q(x,c)]', 'Ax[Q(x,c)]'),
...
def test_to_prenex_normal_form(debug=True):
    for formula,pnf in [
        ('Q(x,c)', 'Q(x,c)'),
        ('Ax[Q(x,c)]', 'Ax[Q(x,c)]'),
